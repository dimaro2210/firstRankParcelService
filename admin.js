...
function findClosestWarehouse(locationString) {
  ...
  // Fallback: match by city/location name
  ...
  for (const warehouse of DISTRIBUTION_WAREHOUSES) {
    ...
    // Check if any significant word from location matches warehouse
    const words = locationLower.split(/[,\s]+/).filter(w => w.length > 3);
    for (const word of words) {
      if (warehouseLoc.includes(word) || warehouseName.includes(word)) {
        return warehouse;
      }
    }
  }
...
function getRouteDistributionCenters(originAddress, destinationAddress) {
  ...
  const routeCenters = DISTRIBUTION_WAREHOUSES.filter(warehouse => {
    ...
    // Only include if within ~3 degrees (~330km) of the direct route
    if (dist > 3.0) {
      return false;
    }

    // Check if warehouse is between origin and destination (not past endpoints)
    const minLat = Math.min(oLat, dLat) - 2;
    const maxLat = Math.max(oLat, dLat) + 2;
    const minLng = Math.min(oLng, dLng) - 2;
    const maxLng = Math.max(oLng, dLng) + 2;
...
async function openEditShipment(trackingCode) {
  ...
  // Populate distribution center dropdown with route-relevant warehouses
  ...
  // Use sender/receiver ADDRESSES (not origin/destination country codes) for better geographic filtering
  // The addresses contain state/city info like "123 Main St, Sacramento, CA" which gives more precise location matching
  const originAddress = shipment.sender?.address || shipment.origin || '';
  const destinationAddress = shipment.receiver?.address || shipment.destination || '';

  // Get distribution warehouses along the route from origin address to destination address
  const routeWarehouses = getRouteDistributionCenters(originAddress, destinationAddress);

  routeWarehouses.forEach(w => {
